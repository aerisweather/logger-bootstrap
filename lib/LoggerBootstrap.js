...
function LoggerBoostrap(opts) {
	...
	// Configure loggly transport options
	...
	function ElasticsearchTransport(options) {
		...
		const defaultTransformer = require('winston-elasticsearch/transformer');
	...
	const config = {
		appLog: {
			...
			elasticsearch: {
				...
				indexPrefix: env('LOG_ELASTICSEARCH_INDEX')
			}
		},
		requestLog: {
			file: {
			...
			elasticsearch: {
				...
				indexPrefix: env('REQUEST_LOG_ELASTICSEARCH_INDEX')
			}
		}
	};
