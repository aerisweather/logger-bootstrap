...
function LoggerBoostrap(opts) {
    ...
    function envFlag(key, defaultVal) {
        ...
        return envVal === null ? defaultVal : (envVal === '1' || envVal === "true" || envVal === "True");
    }
    ...
    const getFormatter = (opts) => {
        ...
        // Add our stack traces automatically
        const errorStackFormatter = winston.format(info => {
            if(info.error && info.error.stack) {
                info.message = `${info.message}, Stack trace:\n${info.error.stack}`;
            }
            return info;
        });

        // Setup our formatters, append to them according to our options.
        const formatters = [
            format.timestamp(),
            errorStackFormatter()
        ];
        ...
        if (opts.timestamp) {
            if (opts.showLevel) {
                formatters.push(timestampLevelFormat);
            }
            else {
                formatters.push(timestampFormat);
            }
        }
        else if(opts.showLevel) {
            formatters.push(levelFormat);
        }
        return winston.format.combine.apply(null, formatters)
    };
    ...
    function ElasticsearchTransport(options) {
        ...
        return new WinstonElasticSearch(Object.assign({
            level: 'info',
