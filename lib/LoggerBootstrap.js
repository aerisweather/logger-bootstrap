...
function LoggerBoostrap(opts) {
	...
	const config = {
		...
		errorLog: {
			// Split out error logs into their own error.log file
			file: {
				silent: !envFlag('LOG_FILE_ENABLED', true),
				level: 'error',
			...
			// Errors are already logged to console and loggly via appLog
			console: { silent: true },
			loggly: { silent: true }
		},
		accessLog: {
			file: {
				silent: !envFlag('ACCESS_LOG_FILE_ENABLED', true),
				level: env('ACCESS_LOG_FILE_LEVEL', 'info'),
				filename: path.join(logDir, 'access.log'),
				maxsize: parseInt(env('ACCESS_LOG_FILE_MAX_SIZE', MB * 10)),
				maxFiles: parseInt(env('ACCESS_LOG_FILE_MAX_FILES', 10)),
				timestamp: envFlag('ACCESS_LOG_FILE_TIMESTAMP', true),
				tailable: true,
				formatter: formatterNoLevel
			},
			console: {
				silent: !envFlag('ACCESS_LOG_CONSOLE_ENABLED', true),
				level: env('ACCESS_LOG_CONSOLE_LEVEL', 'info'),
				timestamp: envFlag('ACCESS_LOG_CONSOLE_TIMESTAMP', true),
				formatter: formatterNoLevel
			},
			loggly: Object.assign({}, loggly, {
	...
	const createLogger = logConfig => new winston.Logger({
		transports: []
			...
			.concat(opts.extraTransports)
	});

	const appLog = createLogger(config.appLog);
	const errorLog = createLogger(config.errorLog);
	const accessLog = createLogger(config.accessLog);
